...
def followAction(state, action, problem):
  for successor1, action1, cost1 in problem.getNextStates(state):
...
class HeuristicTest(testClasses.TestCase):
    ...
    def checkHeuristic(self, heuristic, problem, state, solutionCost):
        ...
        for succ, action, stepCost in problem.getNextStates(state):
...
class CornerHeuristicSanity(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        succs = problem.getNextStates(start_state)
        # cornerConsistencyA
        for succ in succs:
